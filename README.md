...
_Всего было 100 вопросов на 90 минут._
...
* [CSS/JS](#cssjsdom)
* [Рефакторинг, оптимизация](#рефакторинг-оптимизация)
...
* [x] response.json() возвращает содержание ответа как объект json
* [x] response.content возвращает содержание ответа как объект bytes
* [ ] response.content возвращает содержание ответа как объект string
* [x] response.text возвращает содержание ответа как объект string

### 12. Выберите варианты, которые предоставляет Python для доступа к байткоду программ

* [x] Для хотя бы единожды выполненных программ в процессе транслирования интерпретатором кода к виду, доступному для
...
* [x] Специальный класс, в котором объявлены методы `＿enter＿` и `＿exit＿`
...
* [x] Одно лидирующее подчёркивание перед названием атрибута предлагает считать параметр используемым внутри класса,
  однако он всё ещё доступен извне.
* [x] Поля с двойным подчёркиванием доступны изнутри класса, но недоступны извне по такому же имени.
* [x] Поля с двойным подчёркиванием всё же доступны по имени вида `_<ClassName>__<fieldName>`
...
* [x] Имена этих методов начинаются и заканчиваются на двойные подчёркивания
* [x] Методы с неявным вызовом
...
* [x] Список может использоваться как стековая структура данных
* [x] `collections.deque` может использоваться как стек
...
* [ ] Нужно сравнить переменную requests со значением 2.21, а переменную gunicorn - итеративно с
  каждым условием
* [x] Установить пакет requests версии 2.21, а пакет gunicorn - любой, доступный в репозитории, версия которого больше
...
4. [ ] DateField и DateTimeField используются для хранения/представления дат и информации о дате/
   времени
5. [x] EmailField используется для хранения и проверки адресов электронной почты
...
* [x] SELECT T2.x FROM T1 LEFT JOIN T2 ON T1.x = T2.x WHERE T1.x IS NULL
* [x] SELECT T2.x FROM T1 RIGHT JOIN T2 ON T1.x = T2.x WHERE T2.x > 2
...
* [x] SELECT name FROM client WHERE state IN ("MB')
...
* [x] создания ресурса на сервере используется метод POST 
...
### 77. Какие общеупотребительные MIME-типы правильно использовать в НТТP заголовоке Accept?
